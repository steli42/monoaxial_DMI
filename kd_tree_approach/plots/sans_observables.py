...
from scipy.integrate import simpson  # For Simpson's rule integration
...
def plot_structure_factors(data_dir, output_image, norm_const, qx_min, qx_max, 
...
def calculate_differential_cross_section(data_dir, polarised=False):
...
def plot_differential_cross_section(data_dir, output_image, qx_min, qx_max, polarised=False, cmap="plasma", vmin=None, vmax=None, log_scale=False):
    qx, qy, sigma = calculate_differential_cross_section(data_dir, polarised)
...
def plot_radial_averages(data_dirs, custom_labels, num_bins, polarised=False, output_image="radial_averages.jpg"):
   
    plt.figure(figsize=(8, 6))

    for data_dir,label in zip(data_dirs,custom_labels):
        # Calculate the differential cross section
        qx, qy, sigma = calculate_differential_cross_section(data_dir, polarised=polarised)
        ...
        plt.plot(q_bin_centers, sigma_radial_avg, label=label)
...
if __name__ == "__main__":
    ...
    for data_dir in data_dirs: 
        ...
        output_image = os.path.join(data_dir,"structure_factors.jpg") 
        plot_structure_factors(data_dir, output_image, norm_const, qx_min, qx_max, data_type='Norm', cmap=color_map, vmin=0, vmax=1000, log_scale=False)

        output_image = os.path.join(data_dir,"unpol_cross_section.jpg") 
        plot_differential_cross_section(data_dir, output_image, qx_min, qx_max, polarised=False, cmap=color_map, vmin=None, vmax=None, log_scale=True)

        output_image = output_image = os.path.join(data_dir,"pol_cross_section.jpg") 
        plot_differential_cross_section(data_dir, output_image, qx_min, qx_max, polarised=True, cmap=color_map, vmin=None, vmax=None, log_scale=True)
 
    custom_labels = [r"$c=1.0$", r"$c=1/\sqrt{2}$", r"$c=0$"]  
    plot_radial_averages(data_dirs, custom_labels, num_bins=100, polarised=False, output_image=os.path.join(base,"radial_averages.jpg"))


 
# if run in REPL from the plots folder
# data_dir = "../out_super_sqrt2"  # we need to go one step back otherwise the script will look for the out_super_sqrt2 folder inside plots
# output_image = "../out_super_sqrt2/structure_factors.jpg"  
# plot_structure_factors(data_dir, output_image)
# output_image = "../out_super_sqrt2/cross_section.jpg"  

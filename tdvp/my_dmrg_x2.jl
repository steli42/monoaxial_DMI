...
function my_dmrg_x(H::MPO, psi0::MPS, sweeps::Sweeps; kwargs...)
    ...
    PH = ProjMPO(H)
    return my_dmrg_x(PH, psi0, sweeps; kwargs...)
end

function my_dmrg_x(Hs::Vector{MPO}, psi0::MPS, sweeps::Sweeps; kwargs...)
    for H in Hs
        check_hascommoninds(siteinds, H, psi0)
        check_hascommoninds(siteinds, H, psi0')
    end
    ...
    PHS = ProjMPOSum(Hs)
    return my_dmrg_x(PHS, psi0, sweeps; kwargs...)
end

function my_dmrg_x(H::MPO, Ms::Vector{MPS}, psi0::MPS, sweeps::Sweeps; weight=true, kwargs...)
    check_hascommoninds(siteinds, H, psi0)
    check_hascommoninds(siteinds, H, psi0')
    for M in Ms
    ...
    return my_dmrg_x(PMM, psi0, sweeps; kwargs...)
end

using NDTensors.TypeParameterAccessors: unwrap_array_type
"""
    my_dmrg_x(H::MPO, psi0::MPS; kwargs...)
    my_dmrg_x(H::MPO, psi0::MPS, sweeps::Sweeps; kwargs...)
...
represented as a matrix product operator (MPO).
    ...
    my_dmrg_x(Hs::Vector{MPO}, psi0::MPS; kwargs...)
    my_dmrg_x(Hs::Vector{MPO}, psi0::MPS, sweeps::Sweeps; kwargs...)
...
each step of the DMRG algorithm when optimizing the MPS.
    ...
    my_dmrg_x(H::MPO, Ms::Vector{MPS}, psi0::MPS; weight=1.0, kwargs...)
    my_dmrg_x(H::MPO, Ms::Vector{MPS}, psi0::MPS, sweeps::Sweeps; weight=1.0, kwargs...)
    ...
    for sw in 1:nsweep(sweeps)
        sw_time = @elapsed begin
            ...
            for (b, ha) in sweepnext(N)
                ...
                energy = conj.(phi)'*contracted_operator*phi
                vals = diag(vals)
                # @show vals .- energy[1]
                # order = sortperm(real.(vals.-energy[1]))[1:10]
                # @show order

                u_ind = uniqueind(vecs, contracted_operator)
                # vecs = [vecs * dag(onehot(eltype(vecs), u_ind => o)) for o in order]
                # vals = vals[order]

                fids = map(abs, array(phi * dag(vecs)))

                rele = real.(vals .- energy[1])
                # pos = rele .> 0
                # @show pos
                # window = sortperm(abs.(rele))[1:min(4,length(vals))]
                # @show vals[window]
                # order = reverse(sortperm(fids[window]))
                # max_index = 2
                # @show vals
                # max_index = order[2]
...
function my_dmrg_x(
    ...
    return my_dmrg_x(x1, psi0, _dmrg_sweeps(; nsweeps, maxdim, mindim, cutoff, noise); kwargs...)
end

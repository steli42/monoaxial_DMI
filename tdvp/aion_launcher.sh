...
# create the list of config files for the job
find configs/ -path "*.json" | sort > all_configs.txt
...
JOBLOGFILE=logs/$SLURM_JOB_NAME-$SLURM_JOB_ID.log
...
# TASK=${TASK:=${HOME}/GNU_PARALLEL/git/sequential/basics/scripts/run_stressme}
...
PARALLEL="${PARALLEL} --delay .1 -j ${SLURM_NTASKS} --joblog ${JOBLOGFILE}"
# PARALLEL="${PARALLEL} -j0 -N 252 --pipe parallel -j0 --delay .1 -j ${SLURM_NTASKS} --joblog ${JOBLOGFILE}"
